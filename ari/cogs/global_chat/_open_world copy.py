...
class OpenWorldServer(commands.Cog):
    ...
    async def on_message_delete(self, message:discord.Message):
        ...
        if message.id in self.bypass_delete_listener:
            return
    ...
    async def delete_message_by_mods(self, ctx):
        ...
        self.bypass_delete_listener.add(message_id)
        ...
        try:
            ...
            # Delete the main referenced message
            async with aiohttp.ClientSession() as session:

                try:
                    webhook = Webhook.from_url(message_id["webhook"],session=session)
                    await webhook.edit_message(
                        message_id,
                    ...
                    try:
                        if ctx.message.reference:
                            await ctx.message.reference.delete()
            ...
            self.bypass_delete_listener.remove(message_id)
