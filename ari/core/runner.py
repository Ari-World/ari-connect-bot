"""Process-level startup/shutdown orchestration: the event loop, exception
handlers, and graceful-shutdown logic that used to live directly in
`__main__.py`. `ari/__main__.py` is now just a thin shim that calls
`main()` here — the actual entry point convention (top-level `__main__.py`,
required for `python __main__.py` to work) stays put, but the real logic
lives with the rest of the bot chassis.
"""
import asyncio
import functools
import logging
...
async def run_bot(ari: Ari) -> None:
    ...
    token = app_config.discord_api_token
    prefix = app_config.discord_command_prefix
...
def main():
    ...
    try:
        ...
        ari = Ari(prefix=app_config.discord_command_prefix)
